...
size_t			get_type_neg(const char *str)
{
	if (cmp_begin("sphereneg", str) || cmp_begin("SPHERENEG", str))
		return (SPHERENEG);
	else if (cmp_begin("planeneg", str) || cmp_begin("PLANENEG", str))
		return (PLANENEG);
	else if (cmp_begin("coneneg", str) || cmp_begin("CONENEG", str))
		return (CONENEG);
	else if (cmp_begin("cylinderneg", str) || cmp_begin("CYLINDERNEG", str))
		return (CYLINDERNEG);
	else if (cmp_begin("diskneg", str) || cmp_begin("DISKNEG", str))
		return (DISKNEG);
	else if (cmp_begin("paraboloidneg", str) || cmp_begin("PARABOLOIDNEG", str))
		return (PARABOLOIDNEG);
	return (UINT64_MAX);
}

size_t			get_type_uni(const char *str)
{
...
size_t			get_type_other(const char *str)
{
	if (cmp_begin("cam", str) || cmp_begin("CAM", str))
		return (CAM);
